...
class DMN(object):
	def __init__(self,hid_dim,bptt_truncate = -1):
		...
		self.U0_i = normal_param(std=0.02, shape=(self.hidden_dim, word_vector_size))
		self.U1_i = normal_param(std=0.02, shape=(self.hidden_dim, word_vector_size))
		self.U2_i = normal_param(std=0.02, shape=(self.hidden_dim, word_vector_size))
		self.W0_i = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.W1_i = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.W2_i = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		...
		self.W1 = normal_param(std=0.0028, shape=(3*self.hidden_dim, (7*self.hidden_dim)+2))
		self.W2 = normal_param(std=0.04, shape=(1,3*self.hidden_dim))
		self.b1 = constant_param(value=0.0, shape=(3*self.hidden_dim,))
		self.b2 = constant_param(value=0.0, shape=(1,))
		self.Wa = normal_param(std=0.02, shape=(2,self.hidden_dim))
		self.Wb = normal_param(std=0.02, shape=(self.hidden_dim,self.hidden_dim))		

		#self.gru_em = GRUblock(hidden_dim,1)	
		self.U0_em = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.U1_em = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.U2_em = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.W0_em = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.W1_em = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		self.W2_em = normal_param(std=0.02, shape=(self.hidden_dim, self.hidden_dim))
		...
		epm,epm_updates = theano.scan(fn = self.step_em,non_sequences=[s_a,q_q], outputs_info=[None,q_q.copy()],n_steps=1) 
		last_mem = epm[1][-1]	

		self.pred = T.nnet.softmax(T.dot(self.Wa,last_mem))
		...
		self.params = [self.U0_i,self.W0_i,self.b0_i,
				...
				self.W1,self.W2,self.b1,self.b2,self.Wb,self.Wa]
	...
	def train(self,tr_q,tr_a,tr_t,itr):
		...
		for j in range(0,itr):
			...
			for i in range(0,l):
				...
				print "******************"
				if ((i+1)%10 == 0):
					fname = 'states_ans_loc/DMN3.epoch%d' %(j)
...
for m in range(0,12):
	...
	file_specs = 'states_ans_loc/specs3.txt'
...
"""
for j in range(2,13):
	dmn.load_state("states_ans_loc/DMN3.epoch%d"%j)
	pred_t = []
	for i in range(0,len(q_test)):
		...
		pred_t.append(l)
	with open('result%d.csv'%j, 'w') as fp:
