...
class GRUTheano(object):   				#create provision to make multi layer GRUs
	...
	def theano_build(self):
		...
		def f_prop_step(x_t,s_tm1):
			x_e = E[:,x_t]
			z_t = T.nnet.hard_sigmoid(U[0].dot(x_e) + W[0].dot(s_tm1) + b[0])
			r_t = T.nnet.hard_sigmoid(U[1].dot(x_e) + W[1].dot(s_tm1) + b[1])
		    	h_t = T.tanh(U[2].dot(x_e) + W[2].dot(s_tm1 * r_t) + b[2])
		    	s_t = (T.ones_like(z_t) - z_t) * h_t + z_t * s_tm1
			o_t = T.nnet.softmax(V.dot(s_t) + c)[0]
			return [o_t,s_t]

		[o,s],updates = theano.scan(f_prop_step,sequences=x,truncate_gradient=self.bptt_truncate,
					    outputs_info=[None,dict(initial=T.zeros(self.hidden_dim))])
		pred = T.argmax(o,axis=1)
		cost = T.sum(T.nnet.categorical_crossentropy(o,y))    # add regularization
		...
		self.predict_class = theano.function([x], pred)
		self.ce_error = theano.function([x, y], cost)
		self.bptt = theano.function([x, y], [dE, dU, dW, db, dV, dc])

		# rmsprop used : experiment with others ( make or use classes )
...
class DMN(object):
	def __init__(self,hidden_dim,vocab_size,batch_size = 20,bptt_truncate=-1,tm=5):
		...
		self.gru_em = GRUblock(hidden_dim,1)				#change init to normal distribution
		...
		marked = T.ivector()
		y = T.iscalar()
		[o_p,s_p],updates = theano.scan(self.gru_p.calc_next_state,sequences=p,truncate_gradient=self.bptt_truncate,		
						    outputs_info=[None,dict(initial=T.zeros(self.hidden_dim))])
		[o_q,s_q],updates = theano.scan(self.gru_p.calc_next_state,sequences=q,truncate_gradient=self.bptt_truncate,
							outputs_info=[None,dict(initial=T.zeros(self.hidden_dim))])
		...
		for iter in range(1,self.tm+1):
			...
			[o,app] = self.gru_em.calc_next_state(current_episode,memory[iter-1])
			memory.append(app)
		last_mem = memory[-1] 
		if answer_mode == 'feedforward':
			prediction = T.nnet.softmax(T.dot(self.Wa, last_mem))
			...
			def answer_step(a_tm1,pred_tm1):
				...
				pred = T.nnet.softmax(T.dot(self.Wa,s_t))
				return [s_t,pred]
		...
		self.predict = theano.function([p,q,marked],[prediction])		
		loss_ce = T.nnet.categorical_crossentropy(prediction, T.stack([y]))[0] 
		self.f = theano.function([p,q,marked,y],[loss_ce,prediction])
		#another scan for batch training : sequences = data , loss gets added each step
		self.params = [self.gru_p.U0,self.gru_p.W0,self.gru_p.b0,
				self.gru_p.U1,self.gru_p.W1,self.gru_p.b1,
				self.gru_p.U2,self.gru_p.W2,self.gru_p.b2,
				self.gru_em.U0,self.gru_em.W0,self.gru_em.b0,
				self.gru_em.U1,self.gru_em.W1,self.gru_em.b1,
				self.gru_em.U2,self.gru_em.W2,self.gru_em.b2,
		...
		updts = upd.adadelta(loss_ce,self.params)
		#updates_sgd = upd.sgd(loss_ce, self.params, learning_rate=0.0001)
		#updts = upd.apply_momentum(updates_sgd, self.params, momentum=0.9)
		self.train_fn = theano.function(inputs=[p,q,marked,y],outputs=[loss_ce,prediction],updates = updts)
		#def save_state():
	"""
	def new_episode_step(self,c_t,m_im1,q,h_tm1):
		...
		g = T.nnet.sigmoid(l_2)
		[o,gru] = self.gru_em.calc_next_state(c_t,h_tm1)
		gate = g[0]
		h_t = (gate*gru) + ((1-gate)*h_tm1)
		return h_t

	def new_episode(self,c,mem,q):
		e,updates = theano.scan(fn=self.new_episode_step,sequences=c,non_sequences=[mem,q],outputs_info=T.zeros_like(q))
		return e[-1]
	"""
	...
	def train(self,tr_input,tr_q,tr_ans,tr_mask):
		...
		for j in range(0,10):
			...
			for i in range(0,l):
				loss,pred = self.train_fn(tr_input[i],tr_q[i],tr_mask[i],tr_ans[i])
				a_loss=a_loss+loss
				...
				if (i%50 == 0) or (i==999) :
					f = open("states.txt", "w")
					mylist = [x.get_value() for x in self.params]
